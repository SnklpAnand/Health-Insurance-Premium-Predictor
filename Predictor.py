...
model_youth = joblib.load("artifacts\model_youth.joblib")
model_mature = joblib.load("artifacts\model_mature.joblib")
scaler_youth = joblib.load("artifacts\scaler_youth.joblib")
scaler_mature = joblib.load("artifacts\scaler_mature.joblib")
...
def preprocess_input(input_dict):
    ...
    # Defining the expected columns and initializing the DataFrame with zeros
    expected_columns = [
...
def handle_scaling(age, df):
    ...
    if age <= 25:
        scaler_object = scaler_youth
    else:
        scaler_object = scaler_mature
...
def predict(input_dict):
    ...
    if input_dict['Age'] <= 25:
        prediction = model_youth.predict(input_df)
    else:
        prediction = model_mature.predict(input_df)
